...
def find_classes(dir):
    # contains all classes: cat, bird..
    classes = [d for d in os.listdir(dir) if os.path.isdir(os.path.join(dir, d))]
    # sort by abc
    classes.sort()
    # map class to idx: {'bed': 0, 'bird': 1..}
    ...
    for target in sorted(os.listdir(dir)):
        ...
        for root, _, fnames in sorted(os.walk(d)):
            for fname in sorted(fnames):
                # Save the file name to predict the test
                file_names.append(fname)
                if is_audio_file(fname):
                    path = os.path.join(root, fname)
                    item = (path, class_to_idx[target])
                    spects.append(item)
    return spects, file_names
...
def spect_loader(path, window_size, window_stride, window, normalize, max_len=101):
    ...
    # make all spects with the same dims by zero padding
...
class GCommandLoader(data.Dataset):
    ...
    def __init__(self, root, transform=None, target_transform=None, window_size=.02,
        ...
        spects, file_names = make_dataset(root, class_to_idx)
